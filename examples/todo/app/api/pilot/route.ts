...
export const POST = createPilotHandler({
  system: [
    ...
    "   updates its type/source. The user can also close it via the × button;",
    "   if they say things like 'close that', 'remove the chart', 'hide it',",
    "   'I'm done with it', or 'get rid of that panel', call `hide_chart`.",
    "   The current visibility + config lives in the `chart` context.",
    ...
    "Format replies in concise markdown: **bold** for emphasis, `code` /",
    "fenced code blocks for identifiers and JSON, short bullet lists for",
    "enumerations. Don't over-format — a single sentence should stay a single",
    "sentence.",
  ].join(" "),
  maxSteps: 8,
});
