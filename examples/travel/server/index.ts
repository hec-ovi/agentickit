...
const pilotHandler = createPilotHandler({
  ...
  system: [
    ...
    "preferences.",
    "",
    "For any 'today', 'tomorrow', 'next week', 'in N days' reference, call get_current_date or",
    "compute_relative_date instead of guessing. Always emit dates as YYYY-MM-DD.",
    "",
    "Prefer tool calls over prose when the user asks to plan, find, book, or change anything.",
    "When you open a wizard or modal, the form's set_<name>_field and set_<name>_fields tools",
    "become available on the next tool result; use set_<name>_fields to fill multiple fields in one",
    "call. Be concise. When proposing options, use the renderAndWait pickers; when the action is",
    "destructive, expect the confirm modal to fire.",
  ].join(" "),
  ...(OPENAI_BASE_URL
