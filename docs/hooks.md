...
Bind a `react-hook-form` instance to the AI. The AI gets three auto-tools per form: `set_<name>_field`, `submit_<name>`, `reset_<name>`.
...
- `set_contact_field({ field, value })` - writes one field with RHF validation. The field name is validated against the registered fields, so the AI can't write to paths that don't exist.
...
If you don't pass `{ name: "..." }` the suffix defaults to `form` (so `set_form_field`, `submit_form`, `reset_form`).
...
You can call all three hooks in the same component, or split them across the tree. The registry is global to the surrounding `<Pilot>`, so the AI sees the union of everything registered anywhere inside it.
