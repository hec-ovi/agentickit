...
- **CLI `--version` now reports the real package version.** The bin had `const VERSION = "0.0.0"` hardcoded since the first commit, so every published release shipped with `agentickit --version` returning `agentickit 0.0.0`. The test only matched `/agentickit \d/` so the regression silently survived 0.1.0 and 0.2.0. Replaced with a runtime read of the bundled `package.json` (works in dev source and in the bundled bin since both sit one directory below `package.json`); test now asserts exact equality with `package.json`'s `version` field so any future drift fails CI immediately.

### Added, framework

- **`usePilotForm(form, { confirm: { submit?, reset? } })`**: per-form opt-out for the confirm-modal gate on the auto-registered `submit_<name>` and `reset_<name>` tools. Both default to `true` so existing behaviour is unchanged. Set `confirm: { submit: false }` for low-stakes flows (e.g. a draft-only "create" wizard) where the approval popup is friction more than safety. Only the auto-registered tools are affected; consumer actions registered via `usePilotAction` keep the `mutating` flag they declare themselves. 7 tests cover the snapshot-level mutating-flag flip per option plus the end-to-end dispatch path on both submit and reset.
- **CLI `agentickit list-tools` + `add-tool <name>`**: a stock-tools system. The CLI scaffolds opinionated tool plugins from `templates/tools/<name>/` straight into the consumer's repo. Files copy by manifest, env-var stubs append to `.env.example` (idempotent on re-run), refuses to overwrite existing files. First stock tool: `web-search` (see "Added, examples" below). 13 dedicated tests pin the file copy, env-var append, conflict refusal, and manifest "next steps" output. Templates ship via the package's `files` allowlist.
- **CLI `agentickit list-agents` + `add-agent <name> [--type <type>]`**: parallel scaffolder for agent boilerplate. Two stock templates ship: `chat` (concierge pattern: `createPilotHandler` + `<Pilot>` + `<PilotSidebar>`) and `observational` (no-input pattern: SSE-streamed step/log/done events + a status panel with progress bar; for indexing jobs, deployments, etc.). Three placeholders (`{{NAME}}`, `{{NAME_PASCAL}}`, `{{NAME_CAMEL}}`) substitute in both file PATHS and file CONTENTS so a single template generates correctly-named files with correctly-named exports. 14 tests cover placeholder helpers, list-agents shape, add-agent positional + `--type` parsing, default-to-chat behaviour, idempotent re-run guard, and next-steps placeholder substitution.
- **Pretty value renderer for tool-call cards** (`<PilotToolPart>`): humanized key-value rows, 2-column grid with right-aligned small-caps labels, no JSON braces; arrays of uniform-shape objects render as a styled table; booleans become small green/red `yes`/`no` pills; ISO dates render with tabular numerals + a subtle background chip; empty `{}` / `[]` payloads stop showing. Per-section "Pretty / Raw" toggle for power users who want the JSON dump. 11 tests pin the contract (humanization, boolean pill, table rendering, raw-toggle flip, empty-section hiding).
- **Disclosure chevron + smooth open animation on tool cards**: visible 12px CSS-only chevron rotating 90° when the card opens, body fades in via a 160ms keyframe, respects `prefers-reduced-motion`. Cards with no body to expand suppress the chevron entirely. Confirm-modal z-index pinned to int32 max with an override variable so any host UI sits below the approval gate.
- **Theme-friendly tool card layout**: card summary rebuilt as a CSS grid (`[chevron] [humanized title] [status pill]` row 1, `[empty] [raw mono id]` row 2). Title cell owns the only flexible track so the status pill never pushes the title into a mid-phrase wrap. Below 280px the pill drops to its own line via a container query. Raw mono id is suppressed when humanization would just duplicate the title.
- **Open theming surface**: 33 new `--pilot-*` CSS variables covering tool-card chrome (padding, gap, radius, name size/color/weight, line-height), the disclosure chevron (size, stroke, color), the raw-name chip (bg, fg, size, radius, padding), the status pill (size, padding, radius, idle/running/error per-state colors), the pretty value renderer (row gap, label color/size/transform/tracking, table border), and the Raw/Pretty toggle button (default + active state). All defaults declared on `:where(:root)` (specificity 0,0,0,0) so any host `:root { --pilot-*: ... }` override wins automatically. Variables cascade: overriding `--pilot-accent` propagates to the running pill color; `--pilot-error-*` chains into the error pill bg/fg. A `/* ---- Theming surface ---- */` block at the top of the injected stylesheet documents every knob inline so consumers can read the menu in DevTools.

### Added, examples: travel

- **Web-search plugins** (4 backends, each as its own tool the agent picks consciously): DuckDuckGo (no API key, HTML scrape with rate-limit detection), Tavily (`TAVILY_API_KEY`, AI-tuned content-rich snippets), Firecrawl (`FIRECRAWL_API_KEY`, search + scrape), Serper (`SERPER_API_KEY`, Google index, fast). Server-side proxy at `/api/search` dispatches by `?backend=` id; missing-key returns 503 with sign-up instructions; client plugin maps proxy errors to `{ ok: false, reason, code }`. Each tool description carries WHEN-TO-USE / WHEN-NOT-TO-USE / QUERY-CRAFT / OUTPUT rubrics + per-backend "PICK THIS WHEN" guidance. Concierge system prompt gained a WEB SEARCH paragraph that defaults the agent to Serper, Tavily for research, Firecrawl for follow-up scrapes, DuckDuckGo as fallback. 23 hermetic tests + 4 env-gated live integration tests that hit real APIs when keys are set. (Brave + Google CSE were dropped: Brave required payment for the free tier, Google CSE setup is too friction-heavy for plug-and-play.)
- **Read-only SQL plugin over a seeded SQLite catalog**: 3 tables (categories, products, reviews) seeded with ~30 travel-themed products + ~60 reviews. Two new tools: `describe_schema` (introspects tables/columns so the agent doesn't hallucinate names) and `query_products` (runs SELECTs). Three layers of read-only enforcement: a static SQL validator that requires `SELECT` (or `WITH ... SELECT`) and rejects `INSERT/UPDATE/DELETE/DROP/ALTER/CREATE/PRAGMA/ATTACH/REPLACE/TRUNCATE/VACUUM/REINDEX` + statement chaining, a row cap (default 50, max 200) with `truncated: true` flag when the agent over-fetches, and the SQLite engine itself opened in read-only mode. Backed by Node's built-in `node:sqlite` (zero native compile). 17 dedicated tests + manual end-to-end smoke verifying both the happy path (top-3 luggage under $200 by avg rating returns Carry-on Spinner / Packing Cubes / Drybag) and the attack path (`DROP TABLE products` → 400 with clear "only SELECT" reason).
- **Plugin integration tests**: `src/test/render-app.tsx` now exposes a `fireToolCall` helper that captures the runtime's `onToolCall` and synthesizes a tool call, letting tests drive a plugin's action handler exactly the way the runtime would (without spinning up a real model). 13 new tests in `src/plugins/web-search.test.tsx` and `src/plugins/sql.test.tsx` cover the URL-construction contract for each search backend, the structured failure shapes (503 missing-key, 429 rate-limited, network failure), validation/execution failure paths for SQL, and the tool-not-mounted graceful-failure path (dispatcher surfaces a structured error instead of crashing).
...
- **`PilotMessage` and `PilotMessagePart` public types** (vestigial Phase-0 placeholders never used by the framework or any consumer; the runtime works directly with AI SDK 6's `UIMessage`).
- **`PilotSidebarStandalone` component** (defined but never imported anywhere since the sidebar's first commit). Removed from the sidebar file.
- **`examples/todo`**: the legacy showcase app was deleted; `examples/travel` is the canonical example.
